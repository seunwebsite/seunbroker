...
// --- HANDLE 1: UPDATE LIVE PROFIT ---
if (isset($_POST['update_profit'])) {
    $inv_id = intval($_POST['inv_id']);
    $new_profit = floatval($_POST['current_profit']);
    
    $update = mysqli_query($link, "UPDATE investments SET current_profit='$new_profit' WHERE id='$inv_id'");
    
    if ($update) {
        $alert = "Swal.fire({icon: 'success', title: 'Updated', text: 'Live profit updated for user dashboard.'});";
    }
}

// --- HANDLE 2: PAY & END INVESTMENT ---
if (isset($_POST['pay_and_end'])) {
    ...
    if($inv && $inv['status'] == 'active') {
        ...
        mysqli_query($link, "UPDATE users SET balance = balance + $total_payout WHERE id='$user_id'");
        mysqli_query($link, "UPDATE investments SET status='completed', end_date=NOW() WHERE id='$inv_id'");
        
        $alert = "Swal.fire({icon: 'success', title: 'Paid', text: 'Investment closed. Funds added to user balance.'});";
    }
}
                    ...
                    <tbody class="text-sm divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="py-4">
                                ...
                                <span class="text-[10px] bg-slate-100 px-2 py-0.5 rounded uppercase"><?php echo $row['pay_method']; ?></span>
                            </td>
                            <td class="py-4">
                            ...
                            <td class="py-4 pr-6 text-right">
                                <div class="flex justify-end gap-2">
                                    <?php if($row['status'] == 'active'): ?>
                                        ...
                                        <form method="POST" onsubmit="return confirm('Pay out and close this investment?');">
                                    ...
                                    <?php endif; ?>
                                    <form method="POST"><input type="hidden" name="inv_id" value="<?php echo $row['id']; ?>"><button type="submit" name="delete_inv" class="text-red-400"><i class="fa-solid fa-trash"></i></button></form>
                                </div>
                            </td>
                        </tr>
                        <?php endwhile; ?>
